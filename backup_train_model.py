...
class LSTM_MODEL(keras.Model):
    def __init__(self, shape):
        ...
        self.LSTM1 = keras.layers.LSTM(100, return_sequences=True) 
                                        # kernel_regularizer=l2(1e-4),  recurrent_regularizer=l2(1e-4))
        self.BatchNorm3 = keras.layers.BatchNormalization()
        self.LSTM2 = keras.layers.LSTM(200, return_sequences=False) 
                                        # kernel_regularizer=l2(1e-4),  recurrent_regularizer=l2(1e-4))
...
def main():
    ...
    data_dir = 'Train_Company_DailyAdjusted'
    features, labels = [], []
    ...
    model.compile(loss="categorical_crossentropy", optimizer=Adam(learning_rate=0.01), metrics=['accuracy'], run_eagerly=True)
    # model.summary()
